...
const TASKS: Record<string, Tasks> = {
  ...
  [SHOW_MIGRATIONS]: () => {
    shell.exec(
      `${runTypeorm("show")} -d ${datasourcesFullPath[SEEDER_DATA_SOURCE]}`,
    );
    shell.exec(
      `${runTypeorm("show")} -d ${datasourcesFullPath[MIGRATE_DATA_SOURCE]}`,
    );
  },
  [RUN_ALL]: () => {
    shell.exec(
      `${runTypeorm("run")} -d ${datasourcesFullPath[MIGRATE_DATA_SOURCE]}`,
    );
    shell.exec(
      `${runTypeorm("run")} -d ${datasourcesFullPath[SEEDER_DATA_SOURCE]}`,
    );
  },
  [REVERT_ALL]: () => {
    shell.exec(
      `${runTypeorm("revert")} -d ${datasourcesFullPath[SEEDER_DATA_SOURCE]}`,
    );
    shell.exec(
      `${runTypeorm("revert")} -d ${datasourcesFullPath[MIGRATE_DATA_SOURCE]}`,
    );
  },
  [GENERATE_MIGRATION]: async () => {
    ...
    shell.exec(
      `${runTypeorm("generate")} -p -d ${datasourceFullPath} ${outMigration}`,
    );
  },
};
